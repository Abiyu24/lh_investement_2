...
class StoreRequestInherit(models.Model):
    _inherit = 'local.purchase.request'
    
    store_request_id = fields.Many2one('store.request.request', string="Store Request", readonly=True)
    show_create_rfq_button = fields.Boolean(string="Show Create RFQ Button", default=False)
class StoreRequestInheritSiv(models.Model):
    _inherit = 'stock.picking'
           
    store_isue_voucher_id = fields.Many2one('store.request.request', string="Store Request", readonly=True)
    siv_reference = fields.Char(related='store_isue_voucher_id.name', string="SIV Reference", readonly=True, store=True)

class StoreRequest(models.Model):
    _name = 'store.request.request'
    ...
    pr_ids = fields.One2many('local.purchase.request', 'store_request_id', string='PRS')  # Link to related PRs
    ...
    line_ids = fields.One2many('store.request.request.line', 'store_request_line_id', string='Products')
    ...
    state = fields.Selection([
        ('draft', 'Draft'),
        ('dmapproved', 'Department Manager Approved'),
        ('smapproved', 'Store Manager Approved'),
   ...
   # PURCHASE_TYPE_SELECTION = [
    ...
    def create(self, vals):
        if vals.get('reference', _('New')) == _('New'):
            vals['reference'] = self.env['ir.sequence'].next_by_code('store.request.request') or _('New')
    ...
    def action_submit(self):
        ...
        self.schedule_activity_for_group(
            'store_request.group_store_purchase_request_department_manager',
            summary="Store  Request Submitted",
            note="A Store Request request has been submitted and requires review."
        )
    ...
    def action_approve(self):
        ...
        self.schedule_activity_for_group(
            'store_request.group_store_purchase_request_store_manager',
            summary="Store  Request Submitted",
            note="A Store Request request has been submitted and requires review."
        )
    ...
    def action_pm_approve(self):
        ...
        self.schedule_activity_for_group(
            'store_request.group_store_purchase_request_store_kepeer',
            summary="Store  Request Submitted",
            note="A Store Request request has been submitted and requires review."
        )
    ...
    def custom_create_function(self):
        ...
        if self.state != 'storekeeper':
            raise ValidationError(_("You cannot create a PR for a request that is not approved by the storekeeper."))

        # Check if all product demands are greater than available stock
        for line in self.line_ids:
            if line.quantity <= line.product_availability:
                raise ValidationError(_(
                    "You cannot create a PR because product '%s' has enough stock (Stock: %.2f, Demand: %.2f)."
                ) % (line.product_id.display_name, line.product_availability, line.quantity))
    ...
    def custom_create_function(self):
        ...
        pr_values = {
            ...
            'request_department' : self.request_department.id,
            'purchase_type' : self.purchase_type,
            'store_request_id': self.id,  # Link to the purchase request
    }
    ...
    def return_list(self):
        ...
        pr_count = self.env['local.purchase.request'].search_count([('store_request_id', '=', self.id)])
        ...
        return {
            ...
            'view_mode': 'tree,form',
            'domain': [('store_request_id', '=', self.id)],  # Filter by related purchase request
            'target': 'current',
        }
    ...
    def action_create_siv(self):
        """
        Create an SIV (Stock Issue Voucher) from the store request request.
        """
        self.ensure_one()
        ...
        siv_values = {
            ...
            'store_isue_voucher_id': self.id,  # Link to the store request request
        }
        ...
        # Mark the request as having an SIV created
        self.siv_created = True

    def action_view_created_siv(self):
        """
        View the SIVs (Stock Issue Vouchers) linked to this store request request.
        """
        self.ensure_one()
        ...
        return {
            ...
            'view_mode': 'tree,form',
            'domain': [('store_isue_voucher_id', '=', self.id)],  # Filter by related request
            'target': 'current',
        }
...
class StoreRequestLine(models.Model):
    _name = 'store.request.request.line'
    _description = 'Store Request Line'

    store_request_line_id = fields.Many2one('store.request.request', string='Request Reference', ondelete='cascade')
    ...
    def _compute_product_availability(self):
        for record in self:
            ...
            location = record.store_request_line_id.source_location_id

            if product and location:
                # Use with_context to get on-hand qty at the given location
                qty_available = product.with_context({'location': location.id}).qty_available
                record.product_availability = qty_available
            else:
