...
class ForeignCreateRFQ(models.Model):
   ...
   # @api.model_create_multi
    #def create(self, vals_list):
     #   """Create ForeignCreateRFQ records with automatic foreign_reference numbering.
      #  Efficiently handles both single and batch record creation.

       # Args:
        #    vals_list (list/dict): List of value dicts or single value dict
       ...
       # Returns:
        ...
        # Generate sequence numbers in batch
        #if needs_ref:
         #   seq = self.env['ir.sequence']
          #  try:
           #     refs = seq.next_by_code('foreign.create.rfq', len(needs_ref)) or []
            #    # Ensure refs length matches needs_ref, pad with 'New' if necessary
             #   while len(refs) < len(needs_ref):
              #      refs.append(_('New'))
               # # Apply generated references
                #for (i, vals), ref in zip(needs_ref, refs):
                 #   vals['foreign_reference'] = ref
           # except Exception as e:
                # Fallback to individual generation if batch fails
            #    for i, vals in needs_ref:
             #       vals['foreign_reference'] = seq.next_by_code('foreign.create.rfq') or _('New')

        # Validate mandatory fields
       # for vals in vals_list:
        #    if not vals.get('vendor_id'):
         #       raise ValidationError(_("The Supplier field is mandatory. Please select a vendor."))
    ...
    #def create(self, vals_list):
     #   for vals in vals_list:
      #      if not vals.get('vendor_id'):
       #         raise ValidationError(_("The Supplier field is mandatory. Please select a vendor."))
        #    if vals.get('foreign_reference', _('New')) == _('New'):
         #       seq = self.env['ir.sequence'].next_by_code('foreign.create.rfq')
          #      vals['foreign_reference'] = seq or _('New')
       # return super().create(vals_list)
    #@api.model_create_multi
    #def create(self, vals_list):
       # """Create ForeignCreateRFQ records with automatic foreign_reference numbering.
        ...
        # Generate sequence numbers in batch
         ...
         #   seq = self.env['ir.sequence']
          #  refs = seq.next_by_code('foreign.create.rfq', len(needs_ref)) or []
            ...
            # Apply generated references
           # for (i, vals), ref in zip(needs_ref, refs):
            #    vals['foreign_reference'] = ref or _('New')
#
 #       return super().create(vals_list)
